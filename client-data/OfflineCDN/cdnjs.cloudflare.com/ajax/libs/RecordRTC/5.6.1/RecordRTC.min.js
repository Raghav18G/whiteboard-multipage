...
function RecordRTC(mediaStream, config) {
  function startRecording(config2) {
    return (
      config.disableLogs || console.log("RecordRTC version: ", self.version),
      config2 && (config = new RecordRTCConfiguration(mediaStream, config2)),
      config.disableLogs ||
  ...
  function stopRecording(callback) {
    function _callback(__blob) {
      if (!mediaRecorder)
        return void ("function" == typeof callback.call
          ? callback.call(self, "")
          : callback(""));
      Object.keys(mediaRecorder).forEach(function (key) {
        "function" != typeof mediaRecorder[key] &&
      ...
      if (
        ...
        "function" == typeof callback.call
          ? callback.call(self, url)
          : callback(url);
      }
      config.autoWriteToDisk &&
  ...
  function pauseRecording() {
    return mediaRecorder
      ? "recording" !== self.state
        ...
        : (setState("paused"),
          mediaRecorder.pause(),
          void (config.disableLogs || console.log("Paused recording.")))
      : void warningLog();
  }
    ...
    return mediaRecorder
      ? "paused" !== self.state
        ...
        : (setState("recording"),
          ...
          void (config.disableLogs || console.log("Resumed recording.")))
      : void warningLog();
  }
  ...
  function handleRecordingDuration(counter) {
    if (((counter = counter || 0), "paused" === self.state))
      return void setTimeout(function () {
        handleRecordingDuration(counter);
      }, 1e3);
    if ("stopped" !== self.state) {
      if (counter >= self.recordingDuration)
        return void stopRecording(self.onRecordingStopped);
      (counter += 1e3),
        setTimeout(function () {
          handleRecordingDuration(counter);
        }, 1e3);
  ...
  function setState(state) {
    self &&
      ((self.state = state),
      "function" == typeof self.onStateChanged.call
  ...
  var mediaRecorder,
    ...
    returnObject = {
      ...
      setRecordingDuration: function (recordingDuration, callback) {
        if ("undefined" == typeof recordingDuration)
          throw "recordingDuration is required.";
        if ("number" != typeof recordingDuration)
      ...
      clearRecordedData: function () {
        return mediaRecorder
          ? (mediaRecorder.clearRecordedData(),
            void (
              config.disableLogs || console.log("Cleared old recorded data.")
            ))
          : void warningLog();
      },
      getBlob: function () {
      ...
      toURL: function () {
        return mediaRecorder
          ? URL.createObjectURL(mediaRecorder.blob)
          : void warningLog();
      },
      ...
      save: function (fileName) {
        return mediaRecorder
          ? void invokeSaveAsDialog(mediaRecorder.blob, fileName)
          : void warningLog();
      },
      getFromDisk: function (callback) {
        return mediaRecorder
          ? void RecordRTC.getFromDisk(config.type, callback)
          : void warningLog();
      },
      setAdvertisementArray: function (arrayOfWebPImages) {
        config.advertisement = [];
        for (var length = arrayOfWebPImages.length, i = 0; i < length; i++)
      ...
      reset: function () {
        "recording" !== self.state ||
          ...
          mediaRecorder &&
            "function" == typeof mediaRecorder.clearRecordedData &&
            mediaRecorder.clearRecordedData(),
          (mediaRecorder = null),
          setState("inactive"),
          (self.blob = null);
  ...
  return (
    config.recorderType ||
      ...
      (config.audio && config.video
        ? (config.type = "video")
        : config.audio && !config.video && (config.type = "audio")),
    config.recorderType &&
      !config.type &&
      (config.recorderType === WhammyRecorder ||
      config.recorderType === CanvasRecorder ||
      ("undefined" != typeof WebAssemblyRecorder &&
        config.recorderType === WebAssemblyRecorder)
        ? (config.type = "video")
        : config.recorderType === GifRecorder
        ? (config.type = "gif")
    ...
    "undefined" != typeof MediaStreamRecorder &&
      ...
      (config.mimeType || (config.mimeType = "video/webm"),
      config.type || (config.type = config.mimeType.split("/")[0]),
      !config.bitsPerSecond),
    config.type ||
      (config.mimeType && (config.type = config.mimeType.split("/")[0]),
      config.type || (config.type = "audio")),
    config
  );
}
function GetRecorderType(mediaStream, config) {
  var recorder;
  return (
    (isChrome || isEdge || isOpera) && (recorder = StereoAudioRecorder),
    "undefined" != typeof MediaRecorder &&
      "requestData" in MediaRecorder.prototype &&
      !isChrome &&
    ...
    "gif" === config.type && (recorder = GifRecorder),
    "canvas" === config.type && (recorder = CanvasRecorder),
    isMediaRecorderCompatible() &&
      ...
      ("audio" === config.type
        ...
        : "function" == typeof MediaRecorder.isTypeSupported &&
          ...
          (recorder = MediaStreamRecorder)),
    mediaStream instanceof Array &&
      mediaStream.length &&
      (recorder = MultiStreamRecorder),
    config.recorderType && (recorder = config.recorderType),
    !config.disableLogs &&
      recorder &&
      recorder.name &&
      console.log(
    ...
    !recorder && isSafari && (recorder = MediaStreamRecorder),
    recorder
  );
}
  ...
  (this.addStream = function (_mediaStream) {
    ...
    (this.startRecording = function () {
      var recorderType,
        ...
        mimeType = this.mimeType || { audio: null, video: null, gif: null };
      if (
        ("function" != typeof mediaType.audio &&
          isMediaRecorderCompatible() &&
          !getTracks(mediaStream, "audio").length &&
          (mediaType.audio = !1),
        "function" != typeof mediaType.video &&
          isMediaRecorderCompatible() &&
          !getTracks(mediaStream, "video").length &&
          (mediaType.video = !1),
        "function" != typeof mediaType.gif &&
          isMediaRecorderCompatible() &&
          !getTracks(mediaStream, "video").length &&
          (mediaType.gif = !1),
        !mediaType.audio && !mediaType.video && !mediaType.gif)
      )
      ...
      if (
        (mediaType.audio &&
          ...
          mediaType.video || this.audioRecorder.startRecording()),
        mediaType.video)
      ) {
        (recorderType = null),
          "function" == typeof mediaType.video &&
        ...
        if (
          ...
          isFirefox
            ...
            : ((newStream = new MediaStream()), newStream.addTrack(videoTrack));
        }
      ...
      if (mediaType.audio && mediaType.video) {
        ...
        mediaType.audio instanceof StereoAudioRecorder && mediaType.video
          ? (isSingleRecorder = !1)
          : mediaType.audio !== !0 &&
            mediaType.video !== !0 &&
            ...
            (isSingleRecorder = !1),
          isSingleRecorder === !0
            ? ((self.audioRecorder = null), self.videoRecorder.startRecording())
      ...
      mediaType.gif &&
        ((recorderType = null),
        "function" == typeof mediaType.gif && (recorderType = mediaType.gif),
    ...
    (this.getBlob = function (callback) {
      var output = {};
      return (
        this.audioRecorder && (output.audio = this.audioRecorder.getBlob()),
        this.videoRecorder && (output.video = this.videoRecorder.getBlob()),
        this.gifRecorder && (output.gif = this.gifRecorder.getBlob()),
    ...
    (this.save = function (args) {
      (args = args || { audio: !0, video: !0, gif: !0 }),
        args.audio &&
          this.audioRecorder &&
          this.audioRecorder.save(
            "string" == typeof args.audio ? args.audio : ""
          ),
        args.video &&
          this.videoRecorder &&
          this.videoRecorder.save(
            "string" == typeof args.video ? args.video : ""
          ),
        args.gif &&
          this.gifRecorder &&
          this.gifRecorder.save("string" == typeof args.gif ? args.gif : "");
    });
}
...
function invokeSaveAsDialog(file, fileName) {
  ...
  var fileFullName =
    (fileName || Math.round(9999999999 * Math.random()) + 888888888) +
    "." +
    fileExtension;
  ...
  (hyperlink.href = URL.createObjectURL(file)),
    ...
    "function" == typeof hyperlink.click
      ? hyperlink.click()
...
function setSrcObject(stream, element) {
  "srcObject" in element
    ? (element.srcObject = stream)
    : "mozSrcObject" in element
...
function getSeekableBlob(inputBlob, callback) {
  ...
  (fileReader.onload = function (e) {
    ...
    var refinedMetadataBuf = tools.makeMetadataSeekable(
      ...
      newBlob = new Blob([refinedMetadataBuf, body], { type: "video/webm" });
    callback(newBlob);
  }),
...
function MediaStreamRecorder(mediaStream, config) {
  function updateTimeStamp() {
    self.timestamps.push(new Date().getTime()),
      "function" == typeof config.onTimeStamp &&
        config.onTimeStamp(
  ...
  function getMimeType(secondObject) {
    return mediaRecorder && mediaRecorder.mimeType
  ...
  if (
    ((config = config || { mimeType: "video/webm" }), "audio" === config.type)
  ) {
    if (
      getTracks(mediaStream, "video").length &&
      getTracks(mediaStream, "audio").length
    ...
    (config.mimeType &&
      ...
      config.mimeType &&
        "audio/ogg" !== config.mimeType.toString().toLowerCase() &&
        navigator.mozGetUserMedia &&
  ...
  (this.getArrayOfBlobs = function () {
    ...
    (this.record = function () {
      ...
      config.disableLogs ||
        ...
        "function" == typeof MediaRecorder.isTypeSupported &&
          ...
          (MediaRecorder.isTypeSupported(recorderHints.mimeType) ||
            (config.disableLogs ||
              console.warn(
            ...
            (recorderHints.mimeType =
              "audio" === config.type ? "audio/webm" : "video/webm")));
      try {
      ...
      recorderHints.mimeType &&
        ...
        mediaRecorder.canRecordMimeType(recorderHints.mimeType) === !1 &&
        (config.disableLogs ||
          console.warn(
            "MediaRecorder API seems unable to record mimeType:",
            recorderHints.mimeType
          ...
          if (
            ...
            if (!e.data || !e.data.size || e.data.size < 100 || self.blob)
              return void (
                ...
                (self.recordingCallback(
                  new Blob([], { type: getMimeType(recorderHints) })
                ),
                (self.recordingCallback = null))
              );
            (self.blob = config.getNativeBlob
              ? e.data
              : new Blob([e.data], { type: getMimeType(recorderHints) })),
              self.recordingCallback &&
            ...
            updateTimeStamp(),
            "function" == typeof config.ondataavailable)
          ) {
        ...
        (mediaRecorder.onerror = function (error) {
          error &&
            ...
            config.disableLogs ||
              (error.name.toString().toLowerCase().indexOf("invalidstate") !==
              -1
                ? console.error(
                ...
                : "IllegalStreamModification" === error.name
                ? console.error(
                ...
                : "OtherRecordingError" === error.name
                ? console.error(
            ...
            (function (looper) {
              return !self.manuallyStopped &&
                mediaRecorder &&
                "inactive" === mediaRecorder.state
                ? (delete config.timeslice, void mediaRecorder.start(6e5))
                : void setTimeout(looper, 1e3);
            })(),
            "inactive" !== mediaRecorder.state &&
              "stopped" !== mediaRecorder.state &&
              mediaRecorder.stop());
        }),
        "number" == typeof config.timeSlice
          ? (updateTimeStamp(), mediaRecorder.start(config.timeSlice))
    ...
    (this.stop = function (callback) {
      (callback = callback || function () {}),
        (self.manuallyStopped = !0),
        mediaRecorder &&
          ((this.recordingCallback = callback),
          "recording" === mediaRecorder.state && mediaRecorder.stop(),
          "number" == typeof config.timeSlice &&
            setTimeout(function () {
  ...
  (this.getAllStates = function () {
    ...
    "undefined" == typeof config.checkForInactiveTracks &&
      (config.checkForInactiveTracks = !1);
  var self = this;
  !(function looper() {
    if (mediaRecorder && config.checkForInactiveTracks !== !1)
      return isMediaStreamActive() === !1
        ? (config.disableLogs || console.log("MediaStream seems stopped."),
          void self.stop())
        : void setTimeout(looper, 1e3);
  })(),
...
function StereoAudioRecorder(mediaStream, config) {
  ...
  function mergeLeftRightBuffers(config, callback) {
    function mergeAudioBuffers(config, cb) {
      ...
      2 === numberOfAudioChannels &&
        ...
        desiredSampRate && (sampleRate = desiredSampRate);
      var interleaved;
      2 === numberOfAudioChannels &&
        (interleaved = interleave(leftBuffers, rightBuffers)),
        1 === numberOfAudioChannels && (interleaved = leftBuffers);
      var interleavedLength = interleaved.length,
        resultingBufferLength = 44 + 2 * interleavedLength,
        buffer = new ArrayBuffer(resultingBufferLength),
        view = new DataView(buffer);
      writeUTFBytes(view, 0, "RIFF"),
        view.setUint32(4, 36 + 2 * interleavedLength, !0),
        writeUTFBytes(view, 8, "WAVE"),
        writeUTFBytes(view, 12, "fmt "),
        view.setUint32(16, 16, !0),
        view.setUint16(20, 1, !0),
        view.setUint16(22, numberOfAudioChannels, !0),
        view.setUint32(24, sampleRate, !0),
        view.setUint32(28, 2 * sampleRate, !0),
        view.setUint16(32, 2 * numberOfAudioChannels, !0),
        view.setUint16(34, 16, !0),
        writeUTFBytes(view, 36, "data"),
        view.setUint32(40, 2 * interleavedLength, !0);
      for (
        var lng = interleavedLength, index = 44, volume = 1, i = 0;
        i < lng;
        i++
      )
        view.setInt16(index, interleaved[i] * (32767 * volume), !0),
          (index += 2);
      return cb
        ? cb({ buffer: buffer, view: view })
        : void postMessage({ buffer: buffer, view: view });
    }
    if (config.noWorker)
      return void mergeAudioBuffers(config, function (data) {
  ...
  function resetVariables() {
    (leftchannel = []),
      ...
      (intervalsBasedBuffers = { left: [], right: [], recordingLength: 0 });
  }
  function clearRecordedDataCB() {
    jsAudioNode &&
      ...
      (jsAudioNode = null)),
      audioInput && (audioInput.disconnect(), (audioInput = null)),
      resetVariables();
  }
  function onAudioProcessDataAvailable(e) {
    if (!isPaused) {
      if (
        (isMediaStreamActive() === !1 &&
          (config.disableLogs || console.log("MediaStream seems stopped."),
          jsAudioNode.disconnect(),
          (recording = !1)),
        !recording)
      ...
      isAudioProcessStarted ||
        ((isAudioProcessStarted = !0),
        config.onAudioProcessStarted && config.onAudioProcessStarted(),
        config.initCallback && config.initCallback());
      ...
      (recordingLength += bufferSize),
        (self.recordingLength = recordingLength),
        "undefined" != typeof config.timeSlice &&
  ...
  function looper() {
    recording &&
      "function" == typeof config.ondataavailable &&
      "undefined" != typeof config.timeSlice &&
      (intervalsBasedBuffers.left.length
        ? (mergeLeftRightBuffers(
            {
              ...
              rightBuffers:
                1 === numberOfAudioChannels ? [] : intervalsBasedBuffers.right,
            },
  ...
  var jsAudioNode,
    ...
    desiredSampRate = config.desiredSampRate;
  config.leftChannel === !0 && (numberOfAudioChannels = 1),
    1 === config.numberOfAudioChannels && (numberOfAudioChannels = 1),
    (!numberOfAudioChannels || numberOfAudioChannels < 1) &&
      (numberOfAudioChannels = 2),
    config.disableLogs ||
      console.log(
    ...
    "undefined" == typeof config.checkForInactiveTracks &&
      (config.checkForInactiveTracks = !0),
    (this.record = function () {
      if (isMediaStreamActive() === !1)
        throw "Please make sure MediaStream is active.";
      resetVariables(),
        (isAudioProcessStarted = isPaused = !1),
        (recording = !0),
        "undefined" != typeof config.timeSlice && looper();
    }),
    (this.stop = function (callback) {
      (callback = callback || function () {}),
        (recording = !1),
    ...
    "undefined" == typeof RecordRTC.Storage &&
      (RecordRTC.Storage = {
  ...
  var context = RecordRTC.Storage.AudioContextConstructor,
    ...
    bufferSize =
      "undefined" == typeof config.bufferSize ? 4096 : config.bufferSize;
  if (
    (legalBufferValues.indexOf(bufferSize) === -1 &&
      (config.disableLogs ||
        console.log(
  ...
  audioInput.connect(jsAudioNode),
    config.bufferSize || (bufferSize = jsAudioNode.bufferSize);
  var sampleRate =
    "undefined" != typeof config.sampleRate
      ? config.sampleRate
      : context.sampleRate || 44100;
  (sampleRate < 22050 || sampleRate > 96e3) &&
    (config.disableLogs ||
      console.log("sample-rate must be under range 22050 and 96000.")),
    config.disableLogs ||
      (config.desiredSampRate &&
  ...
  (this.pause = function () {
    ...
    (this.clearRecordedData = function () {
      (config.checkForInactiveTracks = !1),
        recording && this.stop(clearRecordedDataCB),
        clearRecordedDataCB();
    }),
  ...
  (jsAudioNode.onaudioprocess = onAudioProcessDataAvailable),
    ...
    (self.recordingLength = recordingLength);
  var intervalsBasedBuffers = { left: [], right: [], recordingLength: 0 };
}
function CanvasRecorder(htmlElement, config) {
  ...
  function drawCanvasFrame() {
    if (isPausedRecording)
      return (
        (lastTime = new Date().getTime()), setTimeout(drawCanvasFrame, 500)
      );
    if ("canvas" === htmlElement.nodeName.toLowerCase()) {
      var duration = new Date().getTime() - lastTime;
      return (
        (lastTime = new Date().getTime()),
        whammy.frames.push({ image: cloneCanvas(), duration: duration }),
        void (isRecording && setTimeout(drawCanvasFrame, config.frameInterval))
      );
    }
    html2canvas(htmlElement, {
      grabMouse:
        "undefined" == typeof config.showMousePointer ||
      ...
      onrendered: function (canvas) {
        ...
        return duration
          ? ((lastTime = new Date().getTime()),
            ...
            void (
              isRecording && setTimeout(drawCanvasFrame, config.frameInterval)
            ))
  ...
  _isChrome &&
    ...
    config.useWhammyRecorder && (isCanvasSupportsStreamCapturing = !1);
  ...
  if (isCanvasSupportsStreamCapturing)
    if (
      ...
      htmlElement instanceof HTMLCanvasElement)
    )
      globalCanvas = htmlElement;
    else {
      if (!(htmlElement instanceof CanvasRenderingContext2D))
  ...
  (this.record = function () {
    if (
      ((isRecording = !0),
      isCanvasSupportsStreamCapturing && !config.useWhammyRecorder)
    ) {
      var canvasMediaStream;
      "captureStream" in globalCanvas
        ? (canvasMediaStream = globalCanvas.captureStream(25))
        : "mozCaptureStream" in globalCanvas
        ? (canvasMediaStream = globalCanvas.mozCaptureStream(25))
        : "webkitCaptureStream" in globalCanvas &&
    ...
    (this.getWebPImages = function (callback) {
      ...
      whammy.frames.forEach(function (frame, idx) {
        ...
        config.disableLogs ||
          ...
          config.onEncodingCallback &&
            config.onEncodingCallback(framesRemaining, framesLength);
        var webp = frame.image.toDataURL("image/webp", 1);
        whammy.frames[idx].image = webp;
      }),
        config.disableLogs || console.log("Generating WebM"),
        callback();
    }),
    (this.stop = function (callback) {
      isRecording = !1;
      var that = this;
      return isCanvasSupportsStreamCapturing && mediaStreamRecorder
        ? void mediaStreamRecorder.stop(callback)
        : void this.getWebPImages(function () {
            whammy.compile(function (blob) {
              config.disableLogs || console.log("Recording finished!"),
                (that.blob = blob),
                that.blob.forEach &&
                  (that.blob = new Blob([], { type: "video/webm" })),
  ...
  (this.pause = function () {
    if (
      ((isPausedRecording = !0),
      mediaStreamRecorder instanceof MediaStreamRecorder)
    )
      return void mediaStreamRecorder.pause();
  }),
    (this.resume = function () {
      return (
        (isPausedRecording = !1),
        mediaStreamRecorder instanceof MediaStreamRecorder
          ? void mediaStreamRecorder.resume()
          : void (isRecording || this.record())
      );
    }),
    (this.clearRecordedData = function () {
...
function WhammyRecorder(mediaStream, config) {
  function drawFrames(frameInterval) {
    ...
    return duration
      ? isPausedRecording
        ? ((lastTime = new Date().getTime()), setTimeout(drawFrames, 100))
        : ((lastTime = new Date().getTime()),
        ...
        var matchPixCount,
          ...
          finishImage = function () {
            (!doNotCheckNext &&
              ...
              (checkUntilNotBlack && (doNotCheckNext = !0),
          ...
          (image.onload = function () {
            ...
            for (var pix = 0; pix < endPixCheck; pix += 4) {
              var currentColor = {
                ...
                colorDifference = Math.sqrt(
                  Math.pow(currentColor.r - sampleColor.r, 2) +
                    Math.pow(currentColor.g - sampleColor.g, 2) +
                    Math.pow(currentColor.b - sampleColor.b, 2)
                );
              colorDifference <= maxColorDifference * pixTolerance &&
                matchPixCount++;
            }
            finishImage();
          }),
  ...
  (config = config || {}),
    config.frameInterval || (config.frameInterval = 10),
    config.disableLogs ||
    ...
    (this.record = function () {
      config.width || (config.width = 320),
        ...
        config.video ||
          (config.video = { width: config.width, height: config.height }),
        config.canvas ||
          (config.canvas = { width: config.width, height: config.height }),
        (canvas.width = config.canvas.width || 320),
        (canvas.height = config.canvas.height || 240),
        (context = canvas.getContext("2d")),
        config.video && config.video instanceof HTMLVideoElement
    ...
    setTimeout(function () {
      dropBlackFrames(whammy.frames, -1, null, null, function (frames) {
        (whammy.frames = frames),
          config.advertisement &&
            config.advertisement.length &&
            (whammy.frames = config.advertisement.concat(whammy.frames)),
          whammy.compile(function (blob) {
            (_this.blob = blob),
              _this.blob.forEach &&
                (_this.blob = new Blob([], { type: "video/webm" })),
...
function GifRecorder(mediaStream, config) {
  ...
  (this.record = function () {
    function drawVideoFrame(time) {
      if (self.clearedRecordedData !== !0) {
        ...
        (lastAnimationFrame = requestAnimationFrame(drawVideoFrame)),
          void 0 === typeof lastFrameTime && (lastFrameTime = time),
          time - lastFrameTime < 90 ||
            (!isHTMLObject && video.paused && video.play(),
            isHTMLObject ||
              context.drawImage(video, 0, 0, canvas.width, canvas.height),
            config.onGifPreview &&
    ...
    return "undefined" == typeof GIFEncoder
      ? void setTimeout(self.record, 1e3)
      : isLoadedMetaData
      ? (isHTMLObject ||
          (config.width || (config.width = video.offsetWidth || 320),
          config.height || (config.height = video.offsetHeight || 240),
          config.video ||
            (config.video = { width: config.width, height: config.height }),
          config.canvas ||
            (config.canvas = { width: config.width, height: config.height }),
        ...
        gifEncoder.start(),
        "function" == typeof config.onGifRecordingStarted &&
          config.onGifRecordingStarted(),
        (startTime = Date.now()),
        (lastAnimationFrame = requestAnimationFrame(drawVideoFrame)),
        void (config.initCallback && config.initCallback()))
      : void setTimeout(self.record, 1e3);
  }),
    (this.stop = function (callback) {
      (callback = callback || function () {}),
  ...
  isHTMLObject &&
    (mediaStream instanceof CanvasRenderingContext2D
      ? ((context = mediaStream), (canvas = context.canvas))
      : mediaStream instanceof HTMLCanvasElement &&
        ((context = mediaStream.getContext("2d")), (canvas = mediaStream)));
...
function MultiStreamsMixer(arrayOfMediaStreams, elementClass) {
  ...
  function drawVideosToCanvas() {
    if (!isStopDrawingFrames) {
      ...
      if (
        ...
        canvas.width =
          videosLength > 1 ? 2 * remaining[0].width : remaining[0].width;
        var height = 1;
        (3 !== videosLength && 4 !== videosLength) || (height = 2),
          (5 !== videosLength && 6 !== videosLength) || (height = 3),
          (7 !== videosLength && 8 !== videosLength) || (height = 4),
          (9 !== videosLength && 10 !== videosLength) || (height = 5),
          (canvas.height = remaining[0].height * height);
      } else
        (canvas.width = self.width || 360),
          (canvas.height = self.height || 240);
      fullcanvas &&
        fullcanvas instanceof HTMLVideoElement &&
  ...
  function drawImage(video, idx) {
    if (!isStopDrawingFrames) {
      var x = 0,
        ...
        height = video.height;
      1 === idx && (x = video.width),
        2 === idx && (y = video.height),
        3 === idx && ((x = video.width), (y = video.height)),
        4 === idx && (y = 2 * video.height),
        5 === idx && ((x = video.width), (y = 2 * video.height)),
        6 === idx && (y = 3 * video.height),
        7 === idx && ((x = video.width), (y = 3 * video.height)),
        "undefined" != typeof video.stream.left && (x = video.stream.left),
        "undefined" != typeof video.stream.top && (y = video.stream.top),
        "undefined" != typeof video.stream.width &&
          (width = video.stream.width),
        "undefined" != typeof video.stream.height &&
          (height = video.stream.height),
        context.drawImage(video, x, y, width, height),
        "function" == typeof video.stream.onRender &&
    ...
    if (
      (arrayOfMediaStreams.forEach(function (stream) {
        if (
          ...
          self.useGainNode === !0 && audioSource.connect(self.gainNode),
  ...
  !(function (that) {
    "undefined" == typeof RecordRTC &&
      ...
      ("undefined" != typeof global.console.log &&
        "undefined" != typeof global.console.error) ||
  ...
  "undefined" == typeof MediaStream &&
    ...
    "undefined" != typeof MediaStream &&
      "undefined" == typeof MediaStream.prototype.stop &&
  ...
  "undefined" != typeof AudioContext
    ...
    (this.appendStreams = function (streams) {
      ...
      streams instanceof Array || (streams = [streams]),
        streams.forEach(function (stream) {
    ...
    (this.releaseStreams = function () {
      (videos = []),
        (isStopDrawingFrames = !0),
        self.gainNode && (self.gainNode.disconnect(), (self.gainNode = null)),
        self.audioSources.length &&
          (self.audioSources.forEach(function (source) {
            source.disconnect();
          }),
          (self.audioSources = [])),
        self.audioDestination &&
          (self.audioDestination.disconnect(), (self.audioDestination = null)),
        self.audioContext && self.audioContext.close(),
...
function MultiStreamRecorder(arrayOfMediaStreams, options) {
  ...
  (options = options || {
    ...
    options.frameInterval || (options.frameInterval = 10),
    options.video || (options.video = {}),
    options.video.width || (options.video.width = 360),
    options.video.height || (options.video.height = 240),
    (this.record = function () {
      (mixer = new MultiStreamsMixer(
        arrayOfMediaStreams,
        options.elementClass || "multi-streams-mixer"
      )),
        getAllVideoTracks().length &&
          ((mixer.frameInterval = options.frameInterval || 10),
          (mixer.width = options.video.width || 360),
          (mixer.height = options.video.height || 240),
          mixer.startDrawingFrames()),
        options.previewStream &&
          "function" == typeof options.previewStream &&
    ...
    (this.clearRecordedData = function () {
      mediaRecorder &&
        (mediaRecorder.clearRecordedData(), (mediaRecorder = null)),
        ...
        try {
          self.recordRTC.stopRecording(function (url) {
            return (
              ...
              self.blob && self.blob.size
                ? void resolve(url)
                : void reject("Empty blob.", self.blob)
            );
          });
        } catch (e) {
          reject(e);
...
function WebAssemblyRecorder(stream, config) {
          ...
          (video.onplaying = function () {
            ...
            var ctx = cvs.getContext("2d"),
              ...
              cameraTimer = setInterval(function () {
                if (
                  (finished && (clearInterval(cameraTimer), controller.close()),
                  first &&
                    ((first = !1),
                    config.onVideoProcessStarted &&
                      config.onVideoProcessStarted()),
                  ctx.drawImage(video, 0, 0),
                  "closed" !== controller._controlledReadableStream.state)
                )
                  try {
                    controller.enqueue(
  ...
  function startRecording(stream, buffer) {
    ...
    config.workerPath || console.error("workerPath parameter is missing."),
            ...
            cameraStream().pipeTo(
              new WritableStream({
                write: function (image) {
                  return finished
                    ? void console.error("Got image, but recorder is finished!")
                    : void worker.postMessage(image.data.buffer, [
                        image.data.buffer,
                      ]);
                },
              })
  ...
  function terminate(callback) {
    return worker
      ? (worker.addEventListener("message", function (event) {
          null === event.data &&
            (worker.terminate(), (worker = null), callback && callback());
        }),
        void worker.postMessage(null))
  ...
  ("undefined" != typeof ReadableStream &&
    ...
    (config.realtime = config.realtime || !0);
  var finished, worker;
  this.record = function () {
    (arrayOfBuffers = []),
      (isPaused = !1),
      ...
      "function" == typeof config.initCallback && config.initCallback();
  };
  var isPaused;
  (this.pause = function () {
    isPaused = !0;
...
(RecordRTC.version = "5.6.1"),
  ...
  (RecordRTC.writeToDisk = function (options) {
    console.log("Writing recorded blob(s) to disk!"),
      (options = options || {}),
      options.audio && options.video && options.gif
  ...
  "undefined" != typeof RecordRTC && (RecordRTC.MRecordRTC = MRecordRTC);
var browserFakeUserAgent =
  "Fake/5.0 (FakeOS) AppleWebKit/123 (KHTML, like Gecko) Fake/12.3.4567.89 Fake/123.45";
!(function (that) {
  that &&
...
"undefined" != typeof AudioContext
  ...
  : "undefined" != typeof webkitAudioContext &&
    (Storage.AudioContext = webkitAudioContext),
  "undefined" != typeof RecordRTC && (RecordRTC.Storage = Storage),
  "undefined" != typeof RecordRTC &&
...
var Whammy = (function () {
  function WhammyVideo(duration) {
    (this.frames = []), (this.duration = duration || 1), (this.quality = 0.8);
  }
  function processInWebWorker(_function) {
    var blob = URL.createObjectURL(
        new Blob(
  ...
  function whammyInWebWorker(frames) {
    ...
    function getClusterData(clusterTimecode, clusterCounter, clusterFrames) {
      return [{ data: clusterTimecode, id: 231 }].concat(
    ...
    function generateEBML(json) {
      for (var ebml = [], i = 0; i < json.length; i++) {
        ...
        "object" == typeof data && (data = generateEBML(data)),
          "number" == typeof data && (data = bitsToBuffer(data.toString(2))),
          "string" == typeof data && (data = strToBuffer(data));
        var len = data.size || data.byteLength || data.length,
          zeroes = Math.ceil(Math.ceil(Math.log(len) / Math.log(2)) / 8),
          sizeToString = len.toString(2),
          padded =
            new Array(7 * zeroes + 7 + 1 - sizeToString.length).join("0") +
            sizeToString,
    ...
    function makeSimpleBlock(data) {
      var flags = 0;
      if (
        (data.keyframe && (flags |= 128),
        data.invisible && (flags |= 8),
        data.lacing && (flags |= data.lacing << 1),
        data.discardable && (flags |= 1),
        data.trackNum > 127)
      )
        throw "TrackNumber > 127 not supported";
      var out =
        [128 | data.trackNum, data.timecode >> 8, 255 & data.timecode, flags]
          .map(function (e) {
  ...
  return (
    (WhammyVideo.prototype.add = function (frame, duration) {
      if (
        ("canvas" in frame && (frame = frame.canvas),
        "toDataURL" in frame &&
    ...
    (WhammyVideo.prototype.compile = function (callback) {
      ...
      (webWorker.onmessage = function (event) {
        return event.data.error
          ? void console.error(event.data.error)
...
"undefined" != typeof RecordRTC && (RecordRTC.DiskStorage = DiskStorage),
  "undefined" != typeof RecordRTC && (RecordRTC.GifRecorder = GifRecorder),
  "undefined" == typeof RecordRTC &&
